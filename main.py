...
try:
    if not firebase_admin._apps:
        if os.path.exists(cred_path):
            ...
            print(f"Firebase conectado desde: {cred_path}")
        else:
            print(f"ERROR: No se encontro credencial en {cred_path}")
...
async def callback(code: str):
    try:
        ...
        if "access_token" not in token_data:
            print(f"Error de autenticacion: {token_data}")
            return HTMLResponse(content=get_html_response("Error de Autenticacion", "#ff0000"), status_code=400)
        ...
        return HTMLResponse(content=get_html_response("VERIFICACION EXITOSA: BIENVENIDO", "#00ffff"))
